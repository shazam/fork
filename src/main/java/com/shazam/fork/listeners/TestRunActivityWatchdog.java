...
public class TestRunActivityWatchdog implements ITestRunListener {
    ...
    public TestRunActivityWatchdog(Configuration configuration, RemoteAndroidTestRunner runner, TestClass test,
                                   String poolName, List<ITestRunListener> testRunListeners, String device) {
		...
		for (String testMethod : methods) {
			for (ITestRunListener testRunListener : testRunListeners) {
				...
				if (previouslyStarted != null) {
					flagTestError(testMethod, previouslyStarted, lastWatchdogFailure + " Tests not run: " + outstandingTests + " : " + methods,
                            asList(testRunListener));
				}
			}
		}
	...
	private void flagTestError(String testMethod, boolean previouslyStarted, String lastWatchdogFailure,
